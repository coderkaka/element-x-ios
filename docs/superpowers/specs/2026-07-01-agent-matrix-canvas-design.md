...
6a. **"Ask agent" / "Start agent task" entry point** — a new action in the timeline item long-press menu, alongside the existing `.replyInThread` (`TimelineItemMenuAction.swift:69`, `TimelineViewModel.swift:547`'s `threadRootEventID`). Reuses the exact same "create a thread rooted at this message" mechanism `.replyInThread` already has; the new action is essentially a relabeled/prefilled entry point into it, so the user doesn't have to first pick "reply in thread" then manually type "@agent, please look into this." Noted here 2026-07-04 (not yet scoped into a spec/plan) — surfaced during a discussion of a third-party "AI-agent-native Matrix workbench" viewpoint, which also independently arrived at threads-as-task-containers (already this catalog's item #6). The main open question if this gets picked up: does the client need to know *which* room member is "the agent" to target (a new room-state-driven "agent binding" concept — bigger scope), or is a simpler naming/mention convention (e.g. always mention `@agent:<homeserver>`) good enough for a personal/small-room use case?
