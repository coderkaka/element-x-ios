...
- **The answer is a normal `m.text` reply, not a new message type.** A dedicated response message type was considered and rejected: unless the client explicitly recognizes a custom `msgtype`, it renders as `UnsupportedRoomTimelineItem` (the existing "unsupported message" fallback) — the exact trap `io.element.agent.turn` had to build interception machinery to avoid. Since a user's answer only needs to look like a normal chat reply (no bespoke rendering is useful for one's own answer), sending plain `m.text` with a human-readable body sidesteps the problem entirely: it renders correctly on every Matrix client, old or new, Element or otherwise, with zero new client code. The machine-readable choice is carried as a non-standard extra JSON key in the same event content, read directly from the raw event by the agent backend — the same technique `io.element.agent.turn` already uses to recover `tool_calls` from `originalJSON` since the Rust SDK only exposes `body` for unrecognized msgtypes.
...
{
  "msgtype": "m.text",
  "body": "Selected: Staging",
  "m.relates_to": {"m.in_reply_to": {"event_id": "$original_request_event_id"}},
  "io.element.agent.choice_selected": ["staging"]
}
```

No new client-side rendering is needed for this event at all — it's a standard text reply. `io.element.agent.choice_selected` is read by the agent backend directly from the raw event, the same way it (or any Matrix bot framework) would read any other custom content field.
...
3. A room member taps (single-select) or toggles-then-confirms (multi-select); the client sends a plain `m.text` reply event with the human-readable `body` and the extra `io.element.agent.choice_selected` field.
